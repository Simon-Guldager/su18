    ...
    public class LevelParser {
        ...
        public void InitLevel(int n) {
            if (n <= numberOfLevels) {
                ...
                GetLevel(levels[currentLevelNumber]);  
                NextCustomer();
            }      
        }
        ...
        private void GetTextMaps() {
            ...
            LevelNames = new string[numberOfLevels];
            
            for (int i = 0; i < numberOfLevels; i++) {
                var file =  Path.Combine(path, files[i].Name);
                levels[i] = file;
                LevelNames[i] = File.ReadAllLines(file)[24].Split(':')[1];
            }
        }
        
        private void GetLevel(string file) {
